      ...
      <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="space-y-5">
          {checkIns.map((item, index) => (
            <ScrollReveal
              key={item.page_item_url}
              animation="fade-up"
              delay={index * 100}
            >
              <div className="group bg-white rounded-2xl shadow-sm border border-gray-100 p-6 pl-8 relative overflow-hidden hover:shadow-lg hover:border-gray-200 transition-all duration-300 hover:-translate-y-0.5">
                ...
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
                  <div className="flex-1">
                    <h3 className="text-lg font-bold text-[#0B1D33] mb-1 group-hover:text-[#0F3460] transition-colors duration-300">
                      {item.data.Title}
                    </h3>
                    <p className="text-sm text-[#94a3b8] mb-3 flex items-center gap-2">
                      <svg
                        className="w-3.5 h-3.5 text-[#00B4D8] shrink-0"
                        fill="currentColor"
                        viewBox="0 0 24 24"
                      >
                        <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z" />
                      </svg>
                      {item.data.CityStateZip} -{" "}
                      <time dateTime={item.data.CreateDateISO}>
                        {item.data.CreateDateDisplay ?? item.data.CreateDate}
                      </time>
                    </p>
                    <p className="text-[#64748B] text-sm line-clamp-2">
                      {stripHtml(item.data.Body)}{" "}
                      <Link
                        href="/checkins"
