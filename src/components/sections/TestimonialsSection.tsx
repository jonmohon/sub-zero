      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-3 gap-8 mb-16">
          {writtenTestimonials.map((t) => (
            <div
              key={t.heading}
              className="group relative bg-white rounded-2xl p-8 shadow-sm border border-gray-100 hover:shadow-xl transition-all duration-500"
            >
        ...
        {/* Video Testimonials — lazy loaded on click */}
        <div className="grid md:grid-cols-2 gap-8">
          {videoTestimonials.map((v) => (
            <VideoTestimonialCard key={v.embedUrl} title={v.title} embedUrl={v.embedUrl} />
          ))}
        </div>
      </div>
    </section>
      ...
      <div className="relative bg-gradient-to-br from-[#0A2540] to-[#0F3460] p-3">
        <div className="relative aspect-[4/3] rounded-xl overflow-hidden">
          {loaded ? (
            ...
            >
              <div className="w-16 h-16 rounded-full bg-[#f89406] flex items-center justify-center shadow-[0_0_30px_rgba(248,148,6,0.4)]">
